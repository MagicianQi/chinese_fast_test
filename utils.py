...
from preprocess import seg, clean_txt
...
def split_train_test(source, output_dir, train_proportion=0.8):
...
def cal_precision_and_recall(classifier, file='static/data_test.txt'):
...
if __name__ == "__main__":
    # 转化成csv
    td = TransformData()
    handler = open('static/data.txt')
    td.to_csv(handler, 'static/data.csv')
    handler.close()

    # 将csv文件切割，会生成两个文件（data_train.txt和data_test.txt）
    train_file, test_file = split_train_test('static/data.csv', auth_data=True)
    ...
    lr = 5
    epoch = 5
    model = f'static/data_dim{str(dim)}_lr0{str(lr)}_iter{str(epoch)}.model'
    ...
    result = classifier.test('static/data_test.txt')
    print(result)
